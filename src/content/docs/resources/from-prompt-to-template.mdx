...
<PromptCard title="Master prompt (blank)" collapse="clamp" prompt={`##############################################
# [TASK NAME]: PROMPT TEMPLATE
# Version: 1.0
# Tested on: [model name], [month and year]
...
This connects directly to the four-part shape taught in [Anatomy of a Good Prompt](/foundations/anatomy-of-a-prompt). It is the same idea carried further: `tone` is the role, `task_description` is the task, `output_format` is the format, and `constraints` is the scrutiny.

<Callout type="note" title="Templates are tuned to a model">
These templates were written for Claude Opus 4.7. When the model changes, a template needs checking. [Keeping a template current with the model](#keeping-a-template-current-with-the-model), at the end of this page, shows how.
</Callout>
...
<PromptCard title="Opportunity Background" collapse="clamp" prompt={`##############################################
...
<PromptCard title="Expert Interview Scorecard" collapse="clamp" prompt={`<context>
...
<PromptCard title="Kickoff Agenda" collapse="clamp" prompt={`# Role
...
<PromptCard title="Functional Diligence Agenda" collapse="clamp" prompt={`# Role
...
One caution, and it is a real one. An agent with standing access to project files is only as safe as the sources it is pointed at. Everything in the [Confidentiality Decision Tree](/best-practices/confidentiality) applies, and it applies to the whole connected source rather than to one document at a time.

<PromptCard title="Project Brain agent instructions" collapse="clamp" prompt={`# Project Brain Agent Instructions
...
A template is tuned to the model it was tested on. A new model reads instructions a little differently, and an old template's output still looks reasonable, so the drift is easy to miss. You can see which model you are running in the model pill, explained in [Thinking & Effort](/power-features/thinking-effort).
...
<PromptCard title="Update a template for the current model" prompt={`Here is a prompt template written for [previous model]. Update it so you, [current model], follow it reliably.
...
These templates are written to be filled with live engagement material. Confirm the document is permitted for processing in the KPMG Enterprise or Team workspace before you attach anything, using the [Confidentiality Decision Tree](/best-practices/confidentiality). And verify the output against the source before it reaches a deliverable, as [Verify Everything](/best-practices/verify-everything) explains.
</Callout>

For shorter, single-task prompts covering everyday deal work, see the [Prompt Library](/resources/prompt-library). For the reasoning behind the structure, and the patterns that stack on top of it, start with [Anatomy of a Good Prompt](/foundations/anatomy-of-a-prompt).
