...
One caution, and it is a real one. An agent with standing access to project files is only as safe as the sources it is pointed at. Everything in the <a href={withBase('/best-practices/confidentiality')}>Confidentiality Decision Tree</a> applies, and it applies to the whole connected source rather than to one document at a time.
...
For shorter, single-task prompts covering everyday deal work, see the <a href={withBase('/resources/prompt-library')}>Prompt Library</a>. For the reasoning behind the structure, and the patterns that stack on top of it, start with <a href={withBase('/foundations/anatomy-of-a-prompt')}>Anatomy of a Good Prompt</a>.
