...
A `.gitignore` only protects files that are not already saved, so before your first push, ask Claude Code to show you exactly what is currently tracked and confirm nothing confidential is in that list. For what may go where, see [Keeping Deal Data Safe](/claude-code/confidentiality).
</Callout>
